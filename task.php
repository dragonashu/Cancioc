...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $dateMatch = explode('/', $date);
        ...
        if(empty($project_id) || empty($title) || empty($date) || empty($time)){
            $error_message = 'Please fill in the required fields: Project, Title, Date and Time';
        } else if (count($dateMatch)!=3 || strlen($dateMatch[0])!=2 || strlen($dateMatch[1])!=2 || strlen($dateMatch[2])!=4 || !checkdate($dateMatch[0],$dateMatch[1],$dateMatch[2])){
            ...
            <form class="form-container form-add" method="post" action="task.php">
                <table>
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="datepicker" name="datepicker" value="<?php echo htmlspecialchars($date); ?>" placeholder="dd/mm/yyyy"/></td>
